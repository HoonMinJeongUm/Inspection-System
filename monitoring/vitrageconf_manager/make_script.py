...
install_agent = "/opt/stack/Inspection-System/install_agent"                    #use for appending texts to script (real path in which script is locate )
path = "/opt/stack/Inspection-System"                                           #path of script
interface_vnf = "eno3"                                                          #replace here with Interface name in VNF
...
def make_script() :
    ...
    open("%s/install_agent" % path, 'w').close()  # make <install_agent> script
    ...
    subprocess.call("echo 'sudo sed -i \"3s/.*/'%s'\tmonitor/g\" \"/etc/hosts\"' >> '%s'" %(host_ip ,install_agent), shell=True)
